...
func calculateBlockHashTestSuite(header BlockHeader, nonce int, output *chainhash.Hash, index int) error {
	const unexpectedResultStr = "CalculateBlockHash #%d got: %s want: %s"

	result := CalculateBlockHash(&header, nonce)
